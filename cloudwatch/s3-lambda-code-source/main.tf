...
resource "aws_ssm_parameter" "s3_input_file" {
  name        = var.name_ssm_parameter_input
  description = "s3 input file ARN"
  type        = "String"
  value       = module.s3_code_replication.bucket_arn_origine[0][0]
  tags        = var.tags
}

resource "aws_ssm_parameter" "s3_input_file_dr" {
  provider    = aws.dr
  name        = "${var.name_ssm_parameter_input}-dr"
  description = "s3 input file ARN"
  type        = "String"
  value       = module.s3_code_replication.bucket_arn_replica[0][0]
  tags        = var.tags
}